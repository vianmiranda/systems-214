...
5. Handling hyphenated words with >= 1 hyphen
    ...
    We want to make sure that our spell checker handles not only 1 hyphen, but more. This ensures that we are traversing through each word in the hyphenated word correctly. "apple-pie", "my-name-is-vian" and "my-favorite-food-is-pizza" are all in `spellcheck/testcases/testdict.txt`, but the rest should report an error
